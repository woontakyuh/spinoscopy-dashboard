...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    // ── Persona/context (call these at session start) ──
    {
      name: "read_persona",
      description:
        "Return ELON.md content — Elon's immutable persona, tone, role, and hard rules. Call this FIRST at the start of every conversation and follow it for all replies. Equivalent to reading the elon://persona resource but callable as a tool.",
      inputSchema: { type: "object", properties: {} },
    },
    {
      name: "get_memory_digest",
      description:
        "Return a pre-formatted text summary of top facts from Elon Memory DB (most important first). Call this at the start of every conversation after read_persona to know what Elon already knows about Tak.",
      inputSchema: {
        type: "object",
  ...
  try {
    ...
    switch (name) {
      ...
      case "get_memory_digest": {
        ...
        const digest = await elonMemory.getMemoryDigest(max)
