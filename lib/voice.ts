...
// ─── MediaRecorder + Whisper 기반 오디오 녹음 ────────────────────
// Web Speech API 버그 (Safari InvalidStateError, Chrome continuous 이상
// 동작 등) 완전 우회. 오디오 파일로 녹음 → 서버 /api/voice/stt → Groq
// Whisper로 전사. 한·영 auto-detect.
export function useAudioRecording(opts: {
  silenceMs?: number       // 이 시간만큼 조용하면 자동 종료
  minUtteranceMs?: number  // 최소 녹음 시간 (너무 짧은 건 버림)
  rmsThreshold?: number    // 침묵 판정 RMS 기준
  ...
  useEffect(() => {
    ...
    type MRConstructor = typeof MediaRecorder & { isTypeSupported?: (mime: string) => boolean }
    const mrGlobal: MRConstructor | undefined = (window as unknown as { MediaRecorder?: MRConstructor }).MediaRecorder
    setIsSupported(
      !!navigator.mediaDevices?.getUserMedia &&
      typeof mrGlobal !== "undefined",
    )
  }, [])
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      // MediaRecorder
      type MRCtor = typeof MediaRecorder & { isTypeSupported?: (m: string) => boolean }
      const MR = (window as unknown as { MediaRecorder: MRCtor }).MediaRecorder
      const candidates = [
      ...
      const mime = candidates.find((m) => MR.isTypeSupported?.(m) ?? false) ?? ""
      ...
      const monitor = () => {
        ...
        const silenceFor = now - lastVoiceTimeRef.current
        const sessionDur = now - startTimeRef.current

        // 침묵 누적 + 최소 발화 시간 넘으면 자동 종료
        if (silenceFor >= silenceMs && sessionDur >= minUtteranceMs + silenceMs) {
          commitNow()
          return
        }
