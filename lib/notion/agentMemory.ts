...
export interface MemoryModule {
  listMemories: (opts?: MemoryQueryOptions) => Promise<MemoryRow[]>
  getMemoryDigest: (maxRows?: number) => Promise<string>
  createMemory: (input: MemoryCreateInput) => Promise<MemoryRow>
  updateMemory: (input: MemoryUpdateInput) => Promise<void>
  archiveMemory: (pageId: string) => Promise<void>
}

export function createMemoryModule(envKey: string): MemoryModule {
  ...
  async function listMemories(opts: MemoryQueryOptions = {}): Promise<MemoryRow[]> {
    const dbId = getDbId()
  ...
  async function getMemoryDigest(maxRows = 30): Promise<string> {
  ...
  async function createMemory(input: MemoryCreateInput): Promise<MemoryRow> {
    const dbId = getDbId()
  ...
  async function updateMemory(input: MemoryUpdateInput): Promise<void> {
  ...
  async function archiveMemory(pageId: string): Promise<void> {
    await updateMemory({ pageId, status: "archived" })
  }

  return { listMemories, getMemoryDigest, createMemory, updateMemory, archiveMemory }
}
