...
PDF 원본은 센터장님·고용산 교수가 이미 공유 중인 Dropbox 폴더에 저장한다. 워커가 Dropbox API
(`files/upload`)로 공유 폴더에 올리면 Dropbox가 양쪽 로컬로 자동 동기화하므로, 지금의
"받아서 전달"하던 워크플로우와 그대로 일치한다(원본이 두 사람 로컬에 모두 쌓임). API 업로드
방식이라 워커 머신에 Dropbox 데스크톱 클라이언트가 없어도 된다(토큰만 필요).
