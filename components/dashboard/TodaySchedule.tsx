...
export function TodaySchedule() {
  ...
  return (
    <div className="border border-border rounded-xl bg-card p-4 space-y-5">
      {/* 오늘 일정 */}
      <div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {(schedules ?? []).map((item) => (
              <div
                key={item.id}
                className="rounded-lg border border-border bg-muted px-4 py-3 flex items-start justify-between gap-3"
              >
      ...
      {upcomingItem && (
        <div>
          ...
          <div className="rounded-lg border border-border bg-muted px-4 py-3 flex items-start justify-between gap-3">
            <div className="min-w-0">
      ...
      {quickAddErrorMessage && (
        <p className="-mt-3 text-xs text-red-300">오류: {quickAddErrorMessage}</p>
      )}
    </div>
  )
}
