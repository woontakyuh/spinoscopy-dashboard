  ...
  return (
    <div className="space-y-4">
      <div className="border border-border rounded-xl p-4 bg-card space-y-3">
        <p className="text-foreground/90 text-sm font-medium">아이디어 메모</p>
          ...
          {(["patient", "research", "idea"] as MemoCategory[]).map((item) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${
                category === item
                  ? "bg-amber-600 border-amber-500 text-foreground"
        ...
        <textarea
          ...
          className="w-full min-h-32 rounded-lg border border-border bg-muted text-foreground placeholder:text-muted-foreground p-3 text-sm outline-none focus:ring-2 focus:ring-amber-500"
        />
      ...
      <div className="border border-border rounded-xl p-4 bg-card space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-foreground/90 text-sm font-medium">저장된 Draft</p>
          <span className="text-muted-foreground text-xs">{drafts.length}개</span>
        </div>
        ...
        {draftsQuery.isLoading ? (
          ...
          <p className="text-muted-foreground text-sm">저장된 draft가 없습니다.</p>
        ) : (
          <div className="space-y-3">
            {drafts.map((draft) => {
              const expanded = expandedId === draft.id
              return (
                <div key={draft.id} className="border border-border rounded-lg bg-muted/50">
                  <div className="p-3 space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="text-foreground text-sm font-medium">{draft.title}</p>
                      <Badge variant="outline" className={CATEGORY_STYLE[draft.category]}>
                        {CATEGORY_LABEL[draft.category]}
                      </Badge>
                      <span className="text-muted-foreground text-xs">{formatCreatedAt(draft.createdAt)}</span>
                    </div>
                    ...
                    {expanded && (
                      <pre className="whitespace-pre-wrap break-words text-foreground text-xs bg-card border border-border rounded-md p-3 max-h-64 overflow-auto">
                        {draft.markdown}
                      </pre>
                    )}
                  </div>
      ...
      <Dialog open={Boolean(deleteTarget)} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent className="bg-card border-border text-foreground">
          ...
          <DialogFooter>
            <Button variant="outline" className="border-border" onClick={() => setDeleteTarget(null)}>
              취소
            </Button>
