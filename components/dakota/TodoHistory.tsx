    ...
    <div className="space-y-4">
      ...
      <section className="border border-border rounded-xl bg-card p-4">
        ...
        {activeLoading ? (
          <div className="space-y-2">
            <Skeleton className="h-11 bg-muted" />
            <Skeleton className="h-11 bg-muted" />
          </div>
        ) : (activeTodos ?? []).length === 0 ? (
          <p className="text-muted-foreground text-sm">진행 중인 할 일이 없습니다.</p>
        ) : (
          <div className="space-y-2">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    {!isDone && editingId === todo.page_id ? (
                      ...
                      <p
                        className={`text-sm truncate ${isDone ? "line-through text-muted-foreground" : "text-foreground cursor-text hover:text-foreground"}`}
      ...
      <section className="border border-border rounded-xl bg-card p-4">
        ...
        {doneLoading ? (
          ...
          <p className="text-muted-foreground text-sm">완료된 할일이 없습니다.</p>
        ) : (
          <div className="space-y-1.5 max-h-[500px] overflow-y-auto">
            {(doneTodos ?? []).map((todo) => (
              <div key={todo.page_id} className="flex items-center gap-2 rounded-lg border border-border/50 bg-muted/50 px-3 py-2">
                <div className="min-w-0 flex-1">
