...
const SYSTEM_CARDS = [
  {
    ...
    tone: "border-cyan-500/30 bg-cyan-500/10 text-cyan-700 dark:text-cyan-200",
  },
  {
    label: "Execution",
...
export function DakotaCommandCenter() {
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 via-card to-card p-5">
        <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-cyan-700 dark:text-cyan-300/80">Dakota Command Center</p>
            <h1 className="mt-2 text-2xl md:text-3xl font-semibold text-foreground">센터장님의 개인 ERP는 여기서 커집니다.</h1>
          ...
          <div className="grid grid-cols-4 gap-2 text-center">
            <div className="rounded-xl border border-border bg-card/70 px-3 py-2">
              <p className="text-lg font-semibold text-cyan-700 dark:text-cyan-200">{taskCounts.active}</p>
              <p className="text-[11px] text-muted-foreground">진행</p>
            </div>
            <div className="rounded-xl border border-border bg-card/70 px-3 py-2">
              <p className="text-lg font-semibold text-amber-700 dark:text-amber-200">{taskCounts.approvals}</p>
              <p className="text-[11px] text-muted-foreground">승인</p>
            </div>
            <div className="rounded-xl border border-border bg-card/70 px-3 py-2">
              <p className="text-lg font-semibold text-red-700 dark:text-red-200">{taskCounts.blocked}</p>
              <p className="text-[11px] text-muted-foreground">막힘</p>
            </div>
            <div className="rounded-xl border border-border bg-card/70 px-3 py-2">
              <p className="text-lg font-semibold text-emerald-700 dark:text-emerald-200">{taskCounts.completed}</p>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        {SYSTEM_CARDS.map((card) => (
          <div key={card.label} className={`rounded-2xl border p-4 ${card.tone}`}>
            ...
            <h2 className="mt-1 font-semibold text-foreground">{card.value}</h2>
            <p className="mt-2 text-xs leading-relaxed text-foreground/80">{card.detail}</p>
          </div>
        ))}
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[0.9fr_1.1fr] gap-4">
        <div className="space-y-4">
          <div className="rounded-2xl border border-border bg-card p-4">
            <h2 className="font-semibold text-foreground">Command Input</h2>
            <p className="text-xs text-muted-foreground mt-1 mb-4">Dakota가 요청을 받아 적절한 specialist로 라우팅합니다.</p>
            <OrchestratorChat />
          </div>

          <div className="rounded-2xl border border-border bg-card p-4">
            <h2 className="font-semibold text-foreground">Memory Boundary</h2>
            <p className="text-xs text-muted-foreground mt-1">shared core와 specialist local memory가 섞이지 않는지 보는 경계입니다.</p>
            <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
              <div className="rounded-xl border border-border bg-muted/70 p-3">
                <p className="text-muted-foreground text-xs">Shared Core</p>
                <p className="mt-1 text-xl font-semibold text-cyan-700 dark:text-cyan-200">{data?.memoryBoundary?.sharedCoreCount ?? "-"}</p>
              </div>
              <div className="rounded-xl border border-border bg-muted/70 p-3">
                <p className="text-muted-foreground text-xs">Specialist Local</p>
                <p className="mt-1 text-xl font-semibold text-violet-700 dark:text-violet-200">{data?.memoryBoundary ? specialistMemoryCount : "-"}</p>
              </div>
            </div>
            {data?.memoryError && <p className="mt-3 text-xs text-amber-700 dark:text-amber-300">Memory 조회 제한: {data.memoryError}</p>}
          </div>
        </div>
