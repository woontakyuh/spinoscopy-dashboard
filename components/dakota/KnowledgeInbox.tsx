    ...
    <section className="space-y-4">
      <div className="rounded-2xl border border-border bg-card p-4">
        <div className="flex flex-col gap-1">
          <p className="text-xs uppercase tracking-[0.18em] text-violet-700 dark:text-violet-300/80">Knowledge Inbox</p>
          <h2 className="text-lg font-semibold text-foreground">Notion과 Obsidian을 2중 입력하지 않기 위한 분류함</h2>
        ...
        {KNOWLEDGE_LANES.map((lane) => (
          <div key={lane.title} className={`rounded-2xl border p-4 ${lane.tone}`}>
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {lane.items.map((item) => (
                <span key={item} className="rounded-full border border-border bg-background/40 px-2 py-1 text-[11px] text-foreground/90">
                  {item}
                </span>
              ))}
            </div>
            <p className="mt-4 text-xs leading-relaxed text-foreground/80">{lane.rule}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="font-semibold text-foreground">승격 후보</h2>
            <p className="text-xs text-muted-foreground mt-1">Dakota가 장기 저장소로 보낼지 판단해야 하는 항목</p>
          </div>
          <span className="text-[11px] text-muted-foreground">1차 정적 큐</span>
        </div>

        <div className="mt-4 space-y-3">
          {PROMOTION_CANDIDATES.map((candidate) => (
            <div key={candidate.label} className="rounded-xl border border-border bg-muted/70 p-3">
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-sm font-medium text-foreground">{candidate.label}</h3>
              ...
              <p className="mt-2 text-xs text-muted-foreground leading-relaxed">{candidate.reason}</p>
            </div>
          ))}
        </div>
      </div>
